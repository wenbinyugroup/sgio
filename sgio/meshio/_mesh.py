...
class CellBlock:
    def __init__(
        ...
        self.type = cell_type
        """A string denoting the cell type, e.g. 'triangle', 
        out of a given list, e.g., 'line', 'triangle', 'quad', 'tetra', 
        'pyramid', 'wedge', 'hexahedron'
        """

        self.data = data
        """A NumPy 2D ndarray(,dtype=int) of integer indices into the :obj:`Mesh`.points array.
        """
...
class Mesh:
    ...
    def __init__(
        ...
        """ndarray: A 2D NumPy ndarray of point coordinates.
        ...
        self.cells = []
        """list: A list of :obj:`CellBlock` objects.
        """
        ...
        """dict: A dictionary of `key, value`, where

        * `key` is the variable name, e.g., "T", "temperature", "P", "pressure",
        * `value` is a NumPy ndarray of corresponding values for the `points`.
          All values have the shape `(num_points, ...)`.
        
        Examples
        --------

        To retrieve the "temperature" of the 3rd point::
        ...
        * `key` is the variable name, e.g., "T", "temperature", "P", "pressure",
        * `value` is a NumPy ndarray, each being the corresponding values across a `cells`'s :obj:`CellBlock`.

        Examples
        --------

        To retrieve the "temperature" of the 4th cell of the 1st :obj:`CellBlock`::
    ...
    def _inspect(self, outstream=print, append_newline=False):
        r"""Print the data structure. (s.t.)
        """
        outstream('='*60)
        if append_newline: outstream('\n')

        outstream(self.__repr__())
        if append_newline: outstream('\n')
        outstream('-'*40)
        if append_newline: outstream('\n')
        outstream('mesh.__dict__')
        if append_newline: outstream('\n')
        # outstream(self.__dict__)
        # if append_newline: outstream('\n')
        for k, v in self.__dict__.items():
            outstream('{}: {}'.format(k, v))
            if append_newline: outstream('\n')

        outstream('='*60)
        if append_newline: outstream('\n')

        outstream('mesh.points')
        if append_newline: outstream('\n')
        outstream('{}'.format(self.points))
        if append_newline: outstream('\n')
        outstream('-'*40)
        if append_newline: outstream('\n')
        outstream('mesh.point_sets')
        if append_newline: outstream('\n')
        outstream('{}'.format(self.point_sets))
        if append_newline: outstream('\n')
        outstream('-'*40)
        if append_newline: outstream('\n')
        outstream('mesh.point_data')
        if append_newline: outstream('\n')
        outstream('{}'.format(self.point_data))
        if append_newline: outstream('\n')

        outstream('='*60)

        if append_newline: outstream('\n')
        for i, cb in enumerate(self.cells):
            outstream('cell block {} (mesh.cells[{}]) type: {}'.format(i, i, cb.type))
            if append_newline: outstream('\n')
            outstream('{}'.format(cb.data))
            if append_newline: outstream('\n')
        outstream('-'*40)
        if append_newline: outstream('\n')
        outstream('mesh.cell_sets')
        if append_newline: outstream('\n')
        outstream('{}'.format(self.cell_sets))
        if append_newline: outstream('\n')
        outstream('-'*40)
        if append_newline: outstream('\n')
        outstream('mesh.cell_data')
        if append_newline: outstream('\n')
        outstream('{}'.format(self.cell_data))
        if append_newline: outstream('\n')
