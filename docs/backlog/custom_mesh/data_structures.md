(custom_mesh_data_structures)=
# Data Structure Reference

This page provides detailed information about the data structures used in
StructureGene.
...
- **name** (`str`, optional): Name of your structure. Used for file naming.
  Default: `''`
- **sgdim** (`int`, optional): Dimension of the structure gene
  - `1`: 1D beam analysis
  - `2`: 2D plate/shell analysis
  - `3`: 3D solid analysis
- **smdim** (`int`, optional): Dimension of the structural model. Usually
  matches `sgdim`.
- **spdim** (`int`, optional): Dimension of the space containing the structure.
  Defaults to `sgdim` if not specified. For example, a curved beam in 3D space
  would have `sgdim=1` and `spdim=3`.

## Mesh Structure

The mesh uses the [meshio](https://github.com/nschloe/meshio) format:
...
`cells` is a list of `CellBlock` objects. Each block contains elements of the
same type:
...
- **2D elements**: `'triangle'` (3), `'triangle6'` (6), `'quad'` (4), `'quad8'`
  (8), `'quad9'` (9)
- **3D elements**: `'tetra'` (4), `'tetra10'` (10), `'hexahedron'` (8),
  `'hexahedron20'` (20), `'hexahedron27'` (27), `'wedge'` (6), `'pyramid'` (5)

#### Connectivity: Critical Note

The connectivity array **must use 0-based indices** referencing the `points`
array, not original node IDs.
...
This separation allows you to reuse the same material at different orientations
without creating duplicate material definitions.
...
VABS/SwiftComp use numeric material IDs. Sgio handles this automatically, but
