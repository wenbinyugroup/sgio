(custom_mesh_troubleshooting)=
# Troubleshooting Common Issues

This page covers common problems and their solutions when creating
StructureGene from custom mesh data.
...
When building mocombos, ensure every unique `(material, angle)` combination
gets an entry:
...
Material name in mocombos doesn't exactly match a key in `sg.materials`. Often
due to:
...
Elements with different numbers of nodes in the same CellBlock, or validation
errors.

**Cause:**

Trying to put different element types, such as triangles and quads, in one
CellBlock.
...
2. **Review working examples**: {ref}`custom_mesh_quickstart` and
   {ref}`custom_mesh_conversion`
