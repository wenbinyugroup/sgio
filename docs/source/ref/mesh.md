...
- `sgio.FORMAT_ALIASES` — maps format aliases to their canonical name (for
  example `'sc'` to `'swiftcomp'`). {func}`normalize_format_name` applies this
  mapping.
- `sgio.FORMAT_REQUIREMENTS` — maps each canonical format name to its
